$(function () {
  let searchTerm = "Dans Surf Videos";
  let videoList = $("#videoList");
  let apiKey = "AIzaSyDkiAcBsHTPuxF2CoqcvE41pCJzXrQYel4";
  let player = $("#player");
  clearVideoList();
  search(searchTerm);
...
function record(item) {
  $('#recordList').append(`<div class="col-lg-2">
        <button id="recordItem" class="btn btn-outline-primary">${item}</button>
      </div>`)
}

// clickList.on("click", "button", function () {
//   let id = $(this).attr("id");
//   findVideoById(id);
// });